...
local function hasPremiumAccess()
    -- Additional security: Check if player exists and is valid
    ...
    -- Return true only if we successfully got a rank and it's 1 or higher
    return success and rank and rank >= 1 -- Rank 1 is Premium, 0 is Free User
end
...
local function requirePremiumAccess()
    if not hasPremiumAccess() then
        Rayfield:Notify({
            Title = "🔒 Access Denied",
            Content = "This feature requires Premium rank or above.",
            Duration = 3
        })
        return false
    end
...
if hasPremiumAccess() then
    PremiumTab = Window:CreateTab("Premium", "gem")
else
    -- Create a placeholder tab that shows access denied
    PremiumTab = Window:CreateTab("Premium", "gem")
    PremiumTab:CreateSection("🔒 Access Restricted")
    PremiumTab:CreateLabel("This tab is only available to Premium users and above.")
    PremiumTab:CreateLabel("")
    PremiumTab:CreateLabel("🔐 Security Note:")
    PremiumTab:CreateLabel("All premium features are protected by server-side rank verification.")
    PremiumTab:CreateLabel("Attempting to bypass this restriction will not work.")
    
    -- Get current rank for display
    local currentRank = "Unknown"
    local success, rank = pcall(function()
        return Services.Players.LocalPlayer:GetRankInGroup(Config.Admin.GroupId)
    end)
    if success and rank then
        if rank == 0 then
            currentRank = "Free User"
        elseif rank == 1 then
            currentRank = "Premium"
        elseif rank >= 254 then
            currentRank = "Moderator+"
        else
            currentRank = "Rank " .. tostring(rank)
        end
    end
    
    PremiumTab:CreateLabel("Current Rank: " .. currentRank)
    PremiumTab:CreateLabel("Required: Premium (Rank 1) or above")
    
    -- Notify user about access restriction
    task.wait(1) -- Wait a moment for UI to load
    Rayfield:Notify({
        Title = "🔒 Premium Access Required",
        Content = "You need Premium rank or above to access premium features. Current rank: " .. currentRank,
        Duration = 5
    })
end
...
PremiumTab:CreateButton({
    ...
    Callback = function()
        if not State.lastClickedAnimal then
            Rayfield:Notify({
                Title = "Error",
        ...
        local spawnArgs, errorMsg = Util.getSpawnArgs(State.lastClickedAnimal, State.lastClickedSkin)
        ...
        Rayfield:Notify({
            Title = "Godmode",
            Content = "Target: " .. State.lastClickedAnimal .. " | Args: " .. table.concat(spawnArgs, ", "),
            Duration = 2
        })
...
PremiumTab:CreateDropdown({
    ...
    Callback = function(selected)
        ...
        if type(selected) == "table" then
            State.selectedWeaponIndex = tonumber(selected[1]) or 1
        else
            State.selectedWeaponIndex = tonumber(selected) or 1
        end
    end
})
...
PremiumTab:CreateButton({
    Name = "Use Weapon",
    Callback = function()
        local code = weaponIndexToCode[State.selectedWeaponIndex]
        if not code then
            return
        end
        ...
        if success then
            ...
            local function ownsPass(player, passId)
                ...
                local ok, owns = pcall(Services.MarketplaceService.UserOwnsGamePassAsync, Services.MarketplaceService, player.UserId, passId)
                return ok and owns
            end
            
            ownsPass(Services.Players.LocalPlayer, 0)
...
-- NPC Flinging variables (now in State table)
...
PremiumTab:CreateButton({
    ...
    Callback = function()
        ...
        local targetPlayer = Services.Players:FindFirstChild(selectedNPCPlayer)
        ...
        State.npcFlingActive = true
        State.npcFlingThread = task.spawn(function()
                    if Services.Players.LocalPlayer.Character and Services.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local npcHRP = targetNPC:FindFirstChild("HumanoidRootPart")
                if npcHRP then
                Services.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(npcHRP.Position + Vector3.new(0, 2, 0))
                    task.wait(0.5)
                end
            end
            ...
            task.wait(2)
            if Services.Players.LocalPlayer.Character and Services.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
                workspace.CurrentCamera.CameraSubject = Services.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            end
...
PremiumTab:CreateButton({
    ...
    Callback = function()
        State.npcFlingActive = false
        if State.npcFlingThread then
            State.npcFlingThread = nil
        end
    end
})

end -- Close the hasPremiumAccess() check for Premium tab content
...
-- Boss Farming Toggle
PremiumTab:CreateToggle({
    Name = "🎯 Auto Boss Farming",
    CurrentValue = false,
    Callback = function(state)
        State.bossFarmingEnabled = state
        
        if state then
            State.bossFarmingThread = task.spawn(farmBosses)
        else
            if State.bossFarmingThread then
                task.cancel(State.bossFarmingThread)
                State.bossFarmingThread = nil
            end
